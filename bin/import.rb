  ...
  def retrieve_fotos(f, type, lift = false)
    ...
    mapped = images.map do |i|
      ...
      when "www.cheesy.at"
        ...
        # cheesy-gallery doesn't deal well with gifs
        if tgt_path.end_with?('.gif')
          tgt_path = tgt_path.gsub(%r{(_posts|_fotos)}, 'assets')
        elsif tgt_path =~ %r{_posts}
...
if POST_CLEAN
  ...
  Dir.chdir(TARGET_DIR) do
    ...
    def process_file(f, is_gallery, do_fix_links)
      ...
      content = content.gsub(%r{\s*\[(rl\\?_)?gallery( \w+="\d+")*\]\s*}, "")
